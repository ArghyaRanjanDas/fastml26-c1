...
def main():
    ...
    if any(r[3] for r in rows):
        ...
        A("")
        A("**Since stage 7, `EXTRA_FEATURES` has 20 entries, not 8**, so a cache built today")
        A("has `F` of width **31**: the 8 above, then `dxy_ord2/3/4`, then the nine from")
        A("`dxysig`/`pdgId` (`dsig_ptw`, `dsig_sum`, `dsig_ord3`, `dsig_ord4`, `n_dsig_gt3`,")
        A("`n_lep`, `lead_lep_pt`, `lep_pt_frac`, `charged_frac`). `data.effective_event_features(True)`")
        A("is the authoritative order and every cache's `meta.json` records it. The two extra")
        A("parquet leaves are read only when a selected feature needs them, so a cache with")
        A("`--extra-features` off is unchanged and costs the same to build.")
        A("`train300k_c3` / `eval100k_c3` are built and are what `c2_canon3` trains on;")
        A("`train300k_c2` / `eval100k_c2` / `train4M` are the earlier 19-feature version.\n")
        A("All of them are standardized with frozen constants in `data.EXTRA_STANDARDIZE`")
    ...
    if mixture:
        ...
        A("")
        A("Note that **`WJetsToQQ` is a separate process from `WJetsToLNu`**, not a variant of")
        A("it: 200,531 and 200,281 events, and the two together are the W+jets group. Counting")
        A("only the leptonic one would put W+jets at 18% instead of 36%.\n")
        A(f"Grouped, the official background is **QCD {bkg['QCD']/tb*100:.2f}%, tt "
          f"{bkg['tt']/tb*100:.2f}%, W+jets {bkg['Wjets']/tb*100:.2f}%** — against our")
        ...
        A("background is the official metric's dominant one.\n")
        A("The re-weighting is exact rather than approximate, and worth stating as an")
        A("identity: AUC is the probability that a random signal event outranks a random")
        A("background event, and when the background is a mixture that probability is linear")
        A("in the mixture weights —\n")
        A("> `AUC_pooled = Σ_g w_g · AUC_vs_g` = `0.0912·AUC_QCD + 0.5450·AUC_tt + 0.3638·AUC_W`\n")
        A("— so the per-background AUCs already in every summary json are all that is needed;")
        A("no rescoring. Computed both ways (sample weights on the saved scores, and the")
        A("weighted mean of per-group AUCs) the two agree to 1e-5. The rounded weights")
        A("0.09 / 0.55 / 0.36 give a number about 0.0008 low — small, but it is a free")
        A("0.0008, so the exact fractions are used here and in `official_auc()`.\n")
        A("Re-weighting our saved eval scores:\n")
    ...
    ov = load("overlap.json")
    A("### Exactly how `train4M` got its background\n")
    A("**Capped, not resampled.** `stream_process` walks a process directory in sorted")
    A("filename order and stops at that process's budget; if the directory runs out first it")
    A("simply yields what it has. There is no sampling with replacement anywhere in")
    A("`data.py`, and no top-up: each process's budget is independent, so QCD coming up")
    A("short does **not** hand its shortfall to tt or W+jets. QCD was asked for 1,333,333 and")
    A("`train/QCD_HT250toInf` contains 902,592 in total, so all 902,592 were taken, once")
    A("each. Every other process met its budget exactly.\n")
    if ov:
        A("| process | requested | kept | duplicates within `train4M` | also in `eval100k` |")
        A("|---|---|---|---|---|")
        for r in ov:
            A(f"| `{r['process']}` | {r['train_requested']:,} | {r['train_kept']:,} | "
              f"{r['train_internal_duplicates']} | {r['overlap']} |")
        A("")
        A("**No event appears twice in `train4M`, and no `train4M` event appears in the eval")
        A("slice — QCD included.** That is checked on provenance, not on directory names:")
        A("every row carries `source_file` and `source_row` identifying its row in the")
        A("upstream COLLIDE2V dataset, and the intersection of those keys between the events")
        A("each cache actually consumed is empty for all seven processes")
        A("(`physics/overlap.py`, `physics/overlap.json`).\n")
