...
def main():
    ...
    if tt:
        ...
        A("|---|---|")
        for k in ("tt_hadronic", "tt_semilep", "tt_leptonic", "QCD", "Wjets"):
    ...
    if s1:
        ...
        A("| feature | dir | AUC vs tt | vs QCD | vs W+jets | vs all bkg |")
        A("|---|---|---|---|---|---|")
        for r in s1[:20]:
            A(f"| `{r['feature']}` | {r['dir']} | {r['auc_tt']:.4f} | {r['auc_qcd']:.4f} | "
              f"{r['auc_w']:.4f} | {r['auc_all']:.4f} |")
    ...
    if s2:
        ...
        A("| + feature | AUC vs tt | Δ vs tt | AUC overall | Δ overall |")
        A("|---|---|---|---|---|")
        for r in s2["rows"]:
            A(f"| `{r['feature']}` | {r['auc_tt']:.4f} | {r['d_tt']:+.4f} | "
    ...
    if s3:
        ...
        A("| step | added | AUC vs tt | Δ | AUC overall | Δ |")
        A("|---|---|---|---|---|---|")
        for h in s3["history"]:
            A(f"| {h['step']} | `{h['added']}` | {h['auc_tt']:.4f} | {h['d_tt']:+.4f} | "
              f"{h['auc_all']:.4f} | {h['d_all']:+.4f} |")
        A("")
